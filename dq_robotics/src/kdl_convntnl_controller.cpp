...
RowVectorXd DQoperations::spatial2CartPoseError(Matrix<double,8,1>  desired_pose, Matrix<double,8,1>  current_pose)
{
	...
	htm_error = DQoperations::dq2HTM(desired_pose) - DQoperations::dq2HTM(current_pose);
	std::cout << "htm_error = " << htm_error(0,3) << htm_error(1,3) << htm_error(2,3) << std::endl;
	std::cout << "Are they equal!!" << std::endl; 
	norm_error = DQoperations::get_error_screw_param(current_pose, desired_pose,  v_e, w_e);
	pose_error = w_e(0), w_e(0), w_e(0), htm_error(0,3), htm_error(1,3), htm_error(2,3);
	return pose_error;
}
